...
{
    // Send higher nibble
    LCD_DATA = ((cmd >> 4) & 0x0F);      // Set data lines for the higher nibble
    ...
    // Send lower nibble
    LCD_DATA = (cmd & 0x0F); // Set data lines for the lower nibble
    LCD_CMD |= (1 << EN);        // Enable high
    _delay_us(1);                // Short delay
    LCD_CMD &= ~(1 << EN);       // Enable low
...
{
    // Send higher nibble
    LCD_DATA = ((data >> 4) & 0x0F);      // Set data lines for the higher nibble
    ...
    // Send lower nibble
    LCD_DATA = (data& 0x0F); // Set data lines for the lower nibble
    LCD_CMD |= (1 << EN);         // Enable high
    _delay_us(1);                 // Short delay
    LCD_CMD &= ~(1 << EN);        // Enable low
